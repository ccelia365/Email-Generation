package edu.bu.met.cs665.EmailGeneration;

/**
 * Name: Cecilia Chacko
 * Date: 10/17/2022
 * Description: This class defines the ConcreteProduct of the Factory Method pattern.
 */
public class Returning extends Customer {

    /**
     * Method creates a unique email for Returning customers.
     *
     * @return a custom email for Returning customers
     */
    @Override
    public String createEmail() {
        email.setEmailSubject("WELCOME BACK TO OUR BOOKSTORE!");
        email.setEmailBody("Thanks for becoming our customer again. " +
                "Checkout our latest collections!.");
        email.setEmailClosing("Sincerely, Flourish and Blotts");
        return email.toString();
    }
}
